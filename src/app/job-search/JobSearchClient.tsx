...
export default function JobSearchClient({ initialSearchTerm = "" }: JobSearchClientProps) {
  const [keyword, setKeyword] = useState(initialSearchTerm);
  const [activeSearchTerm, setActiveSearchTerm] = useState(initialSearchTerm);

  const { data, error, isLoading } = useQuery<JobAdsResponse>({
    queryKey: ["jobads"],
    queryFn: () => api.get<JobAdsResponse>("/core/live/jobads"),
    staleTime: 60 * 1000, // 1 minute
  });

  const jobListings = data?.items || [];

  // Sync to localStorage for compatibility with job detail caching logic
  useEffect(() => {
    if (jobListings.length > 0) {
      localStorage.setItem("rduk_cached_jobs", JSON.stringify(jobListings));
    }
  }, [jobListings]);
          ...
          <div className="jobs-container">
            ...
            {isLoading ? (
              ...
              <div className="rounded-xl border border-red-200 bg-red-50 p-8 text-xl font-bold text-red-700 shadow-md">
                Unable to load live jobs. Please check your connection or try again later.
              </div>
            ) : (
