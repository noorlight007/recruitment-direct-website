        ...
        <div className="flex items-center justify-between h-20 lg:h-24">
                ...
                item.hasDropdown ? (
                  <div key={item.label} className="relative" ref={item.label === activeDropdown ? dropdownRef : null}>
                    <div className="flex items-center">
                      <Link
                        href={item.href}
                        className="px-2 py-1 text-sm font-medium text-gray-900 hover:text-white transition-colors rounded-lg hover:bg-[#020307]"
                      >
                        {item.label}
                      </Link>
                      <button
                        ...
                        className="p-1.5 ml-1 text-gray-900 hover:text-black transition-colors rounded-lg hover:bg-[#020307]"
                      >
          ...
          <div className="hidden lg:flex items-center gap-3">
            <div className="header-social-icons">
              <a href="https://wa.me/447590882626" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp">
              ...
              <a href="https://www.linkedin.com/company/recruitment-direct/" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn">
                <svg viewBox="0 0 24 24" fill="#0A66C2">
                  <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                </svg>
              </a>
              <a href="https://www.facebook.com/recruitmentdirect/" target="_blank" rel="noopener noreferrer" aria-label="Facebook">
            ...
            <a
              ...
              className="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-[#0c1730] hover:text-[#d3a94a] transition-colors"
            >
              <div className="w-8 h-8 rounded-lg bg-[#0c1730]/10 flex items-center justify-center text-[#0c1730]">
                <Phone className="w-4 h-4" />
              </div>
              <span className="font-bold text-[#0c1730] tracking-tight">01324 613198</span>
            </a>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div
            ...
            className="lg:hidden bg-[#07111f] border-t border-white/10 text-white overflow-hidden"
          >
            <div className="px-4 py-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div className="space-y-1">
                {menuItems.map((item) => (
                  <div key={item.label} className="flex flex-col">
                    {item.hasDropdown ? (
                      <>
                        <div className="flex items-center justify-between w-full rounded-lg hover:bg-white/10 transition-colors">
                          <Link
                            href={item.href}
                            onClick={() => setMobileOpen(false)}
                            className="flex-grow px-4 py-3 text-white font-semibold hover:text-[#d3a94a] transition-colors"
                          >
                            {item.label}
                          </Link>
                          <button
                            ...
                            className="p-3 text-white hover:text-[#d3a94a]"
                          >
                      ...
                      <a
                        href={item.href}
                        onClick={() => setMobileOpen(false)}
                        className="block px-4 py-3 text-white font-semibold rounded-lg hover:bg-white/10 hover:text-[#d3a94a] transition-colors"
                      >
                        {item.label}
                      </a>
                    )}
              ...
              {/* Mobile Socials */}
              <div className="header-social-icons px-4 py-2 border-t border-white/10 pt-4">
                <a href="https://wa.me/447590882626" target="_blank" rel="noopener noreferrer">
                  <svg className="fill-[#25D366]" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                </a>
                <a href="https://www.linkedin.com/company/recruitment-direct/" target="_blank" rel="noopener noreferrer">
                  <svg viewBox="0 0 24 24" fill="#0A66C2">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                  </svg>
                </a>
                <a href="https://www.facebook.com/recruitmentdirect/" target="_blank" rel="noopener noreferrer">
                  <svg viewBox="0 0 24 24" fill="#1877F2">
                    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                  </svg>
                </a>
              ...
              <div className="px-4 pb-2 text-center text-xs text-white/60 font-medium">
                Prefer to speak? Call us on <a href="tel:01324613198" className="hover:text-[#d3a94a] transition-colors">01324 613198</a>
              </div>
            </div>
          </motion.div>
        )}
